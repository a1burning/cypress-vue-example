...
describe('e2e辅助选择方法', function () {
  ...
  context('基础选择方法', function () {
    ...
    it('contains', function () {
      ...
      cy.contains('选择元素命令'); // 可以后面加options
      ...
      cy.contains(/^Li\d+/); // 可以传选择器+文本，返回的对象是第一个参数写的选择器

      cy.contains('#auxiliarySelector', 'Li1'); // 和get搭配链式调用
  ...
  context('父子关系选择方法', function () {
    ...
    it('children', function () {
      // 所有的li标签，返回的是长度为4的数组
      cy.get('#auxiliarySelector').children(); // 单独id是auxId1的li标签，返回的是DOM元素
    ...
    it('parent', function () {
      // 返回父元素ul DOM元素
      cy.get('#auxId1').parent(); // 如果里面写的不是第一个父元素的选择器，就会报错
    ...
    it('parents', function () {
      ...
      cy.get('#auxId1').parents(); // 所有div的父元素，返回长度为5的数组
    ...
    it('parentsUntil', function () {
      ...
      cy.get('#auxId1').parentsUntil('html'); // 如果选择器不在父元素上，则返回所有父元素（长度为8的数组）
    ...
    it('root', function () {
      ...
      cy.get('#auxId1').root(); // 获取到html元素

      cy.get('#auxiliarySelector').find('li').root(); // within可以缩小root范围
    ...
    it('closest', function () {
      ...
      cy.get('#auxId1').closest('li'); // 父级

      cy.get('#auxId1').closest('#auxiliarySelector'); // 祖先
  ...
  context('兄弟关系选择方法', function () {
    ...
    it('siblings', function () {
      // 除自己外所有兄弟元素
      cy.get('#auxId1').siblings(); // 除自己外所有符合条件的兄弟元素
    ...
    it('next', function () {
      cy.get('#auxId1').next();
      cy.get('#auxiliarySelector li').next('.auxClass'); // cy.get('#auxId1').next('#auxId2') // error
    }); // 获取给定DOM之后的所有同级元素
    ...
    it('nextUntil', function () {
      ...
      // 不包括#auxId2
      cy.get('#auxId1').nextUntil('#auxId2'); // 虽然文档有说有filter，实际可用，但是TS类型并不支持，需要去e2e.ts中配置

      cy.get('#auxId1').nextUntil('#auxId3', '#auxId2'); // https://github.com/cypress-io/cypress-documentation/issues/5078
    }); // 获取下一个紧跟着的兄弟元素
  ...
  context('精准选择方法', function () {
    ...
    it('eq', function () {
      // 正数
      cy.get('#auxiliarySelector').children('li').eq(2); // 倒数
    ...
    it('shadow', function () {
      cy.get('#shadow-card').shadow().find('div').contains('Name');
    });
  });
});